...
spec (we use [[http://rspec.info][rspec]]) didn't work exactly the way he expected.  To aid in the
...
When the teammate receives the code, he may want to run it in order to understand it.
In this case, being a spec
file, he will need to paste the code into a running test system
to explore how it works.

That can be a bit of a hassle, especially when using Rails
where specs often do not run that fast.  It's not that
running them is impossible, but it may be sufficiently inconvenient that one wouldn't just
immediately do it.
...
* Solution
#+ATTR_HTML: :target "_blank"
Like the =jsbin=-like workspace, [[http://fkchang.github.io/opal-playground][opal-playground]] has a [[http://fkchang.github.io/opal-playground/rspec/][page]]
configured to run spec tests by pasting them in and executing. Now instead of sending
my co-workers snippets of code, I test the spec in the Opal playground and test it.
When I'm satisfied that it's doing what I want, I send the link directly
to him.  Here's what it looks like:
#+ATTR_HTML: :target "_blank"
...
Note that if you press the blue button, *Run Code*..., that both tests pass.
Notice that the 2nd test's "it" statement says it should fail, but it passes.

* How Technology Helped Solve the Original Problem
The teammate's pull request contained a rspec stating that a method
=#name= must be invoked.
...
As you can see above, the =#name= method is never invoked and yet
the specs still passes.  So I changed the "it" comment to point
this out and sent it to the teammate:
...
- Retainable as a url: he can always go back and review the example
  ...
  it. While collocation is best for communication,
  "executable rspec gists" can help to mitigete the "distance" barrier
  when the teammate is remote.
