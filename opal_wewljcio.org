...
The first thing I noticed was a formerly smallish ~reload()~ method
...
calling out to methods the global ~document~ object.  I want to be
able to inject a /test document/ for the spec, and inject the real
...
objects which won't act the way the javascript objects in question
will.  We'll need to create our own.
...
- ~fake_links_document()~ a convenience method which returns both the
  test double for ~document~ which responds to the
  ~getElementsByTagName('link')~ and the link itself so
